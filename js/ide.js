...
const handleStorageError = (error) => {
    if (error.message.includes('storage is not allowed')) {
        console.warn('Storage access is not available in this context');
        return null;
    }
    throw error;
};
...
function showError(title, content) {
    ...
    // Add AI assistant suggestion
    const messagesContainer = document.querySelector('#chat-messages');
    if (messagesContainer) {
        const errorMessage = document.createElement('div');
        errorMessage.className = 'message ai-message';
        
        if (title === "Error" && content === "Source code can't be empty!") {
            errorMessage.innerHTML = marked.parse(`
### Empty Source Code Detected

I noticed you're trying to run code, but the editor is empty. Here are some suggestions:

1. **Start with a Template**:
   - Use the language selector to choose your preferred language
   - I can help you generate a "Hello World" program
   - Or I can provide a starter template for your specific needs

2. **Import Existing Code**:
   - Use File > Open to load an existing file
   - Or paste your code directly into the editor

Would you like me to help you get started with some sample code? Just let me know what you'd like to build!
            `);
        } else {
            errorMessage.innerHTML = marked.parse(`
### Error Detected
...
Would you like me to help you resolve this error? I can:
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    require(["vs/editor/editor.main"], function (ignorable) {
        ...
        layout.registerComponent("assistant", function (container, state) {
            ...
            const availableModels = [
                ...
                { name: "llama2-70b-4096", displayName: "LLaMA2 70B" },
                { name: "gemma-7b-it", displayName: "Gemma 7B" }
            ];
            
            element.innerHTML = `
                <div class="assistant-container">
                    <div class="model-selector-container">
                        <select id="model-selector" class="ui dropdown">
            ...
            messagesContainer.appendChild(createMessageElement(
                `👋 Hello! I'm your AI coding assistant powered by Groq. How can I help you today?`,
                false
            ));

            // Add event listeners
            const chatInput = element.querySelector('#chat-input');
            const sendButton = element.querySelector('#send-message');
            const modelSelector = element.querySelector('#model-selector');

            // Add diff view
            const diffView = element.querySelector('#diff-view');
            ...
            sendButton.onclick = async () => {
                ...
                try {
                    const response = await groqService.generateCompletion(prompt, currentCode);
                    
                    // Extract code blocks from the response
                    const codeBlocks = extractCodeFromMarkdown(response);
                    
                    if (codeBlocks.length > 0) {
                        try {
                            // Create unified diff
                            const diff = createPatch('code.txt', currentCode, codeBlocks[0]);
                            ...
                            const diffHtml = Diff2Html.html(diff, {
                                ...
                                maxLineSizeInBlockForComparison: 40
                            });
                            
                            // Add the response with the diff view embedded
                            const messageHtml = `
                                ${marked.parse(response.split('```')[0])}
                                <div class="diff-wrapper">
                                    ...
                                    <div class="diff-buttons">
                                        <button class="accept-changes">Accept Changes</button>
                                        <button class="reject-changes">Reject Changes</button>
                                    </div>
                                </div>
                            `;
                            ...
                            const message = document.querySelector('.chat-messages .message:last-child');
                            message.querySelector('.accept-changes').onclick = () => {
                                sourceEditor.setValue(codeBlocks[0]);
                            };
                            
                            message.querySelector('.reject-changes').onclick = () => {
                                // Just ignore the changes
                            };
                            
                        } catch (diffError) {
                            console.error('Error creating diff:', diffError);
                            // Fallback to showing the response without diff
    ...
    async function testGroqConnection() {
        try {
            const response = await getAICompletion("Hello, test message");
    ...
    window.addEventListener('load', testGroqConnection);
});
...
function createMessageElement(content, isUser) {
    ...
    // Use marked to render markdown, and highlight.js for code blocks
...
function applyChanges(original, changes) {
...
// Add this function to create patches manually
